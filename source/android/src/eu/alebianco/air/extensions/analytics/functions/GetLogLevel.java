...
public class GetLogLevel implements FREFunction {

    @Override
    public FREObject call(FREContext context, FREObject[] args) {
        FREObject result;

        Logger.LogLevel level = GoogleAnalytics.getInstance(context.getActivity()).getLogger().getLogLevel();

        try {
            result = FREObject.newObject(level.name());
        } catch(Exception e) {
